...
if(!empty($_POST['Update'])){
    ...
    //处理信件内容
    if(!empty($Template) && !empty($Ready)){
        $TemplateIDAll = 0;
        $name_old = array('[姓名]','[性别]');
        $content = '';
        foreach($Ready as $key=>$ReadyID){
                ...
                if(empty($content) && $_POST['checkall'] == 1){  //全部统一
                    ...
                    if($user_info['Sex'] == '男'){   //变更性别对应的称谓
                        $user_info['Sex'] = '先生';
                    }else{
                        $user_info['Sex'] = '女士';
                    }
                    $name_new = array($user_info['Username'],$user_info['Sex']);
                    
                    $get_template = mysql_query("select `TemplateContent` from `template` where `TemplateID`='$TemplateID' limit 1");
                ...
                if(!empty($TemplateIDAll)){
                    ...
                    if(empty($template_info)){
                        ...
                        $up_ready = mysql_query("update `message_ready` set `Status`='$Status',`Content`='$message',`TemplateID`='$TemplateID',`Date`='$Date' where `ReadyID`='$ReadyID'");
                    }
                }else{
                    $up_ready = mysql_query("update `message_ready` set `Status`='$Status',`Content`='$message',`TemplateID`='$TemplateID',`Date`='$Date' where `ReadyID`='$ReadyID'");
                }   
    ...
    mysql_query("insert into `message`(`ToEmail`,`FromEmail`,`Content`,`Date`) select `ToEmail`,`FromEmail`,`Content`,`Date` from `message_ready` where `Status`='OK'");
                        ...
                        <tbody>
                         ...
                         <?php
                            ...
                            if(empty($_POST['Data'])){
                                ...
                                $get_ready = mysql_query("select * from `message_ready` as a join `user` as b   where a.`ToEmail`=b.`Email`  order by a.`Content` desc  limit $offset,$pernum ");
                            
                            }else{
                                $Data = trim($_POST['Data']);
                                $DataType = trim($_POST['DataType']);
                                if($DataType =='User'){
                                   $findsql = "select * from `message_ready` as a join `user` as b  join `group` as c where a.`ToEmail`=b.`Email`   and b.`Email` like '%$Data%' or b.`Username` like '%$Data%' order by `ReadyID` desc ";
                                    
                                }elseif($DataType =='Template'){
                                   $findsql = "select * from `message_ready` as a join `user` as b  join `group` as c where a.`ToEmail`=b.`Email`   and b.`Content` like '%$Data%' order by `ReadyID` desc ";
                                    
                                }else{
                                    $findsql = "select * from `message_ready` as a join `user` as b  join `group` as c where a.`ToEmail`=b.`Email`   order by `ReadyID` desc ";
                                }

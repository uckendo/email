...
if(!empty($_POST)){
    ...
    if(empty($Email) || empty($Smtp) || empty($Password)){
        ...
        if(!empty($AccountID)){ //编辑
            $up = mysql_query("update `account` set `Email`='$Email',`Name`='$Name',`Smtp`='$Smtp',`Password`='$Password',`Status`='$Status' where `AccountID`='$AccountID'");
            if($up){    //更新成功
            ...
            if(mysql_num_rows($check)>0){
                ...
                $insql = "insert into `account`(`Email`,`Smtp`,`Password`,`Status`,`Name`)values('$Email','$Smtp','$Password','$Status','$Name')";
            ...
            <div class="row form-wrapper">
                ...
                <div class="col-md-8 column">
                        <div class="field-box">
                            <label>发件人:</label>
                            <div class="col-md-7">
                                <input class="form-control" type="text" name='Name' value="<?=$account_info['Name']?>"/>
                            </div>                            
                        </div>
